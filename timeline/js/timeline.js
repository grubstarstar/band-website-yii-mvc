var Timeline = function(container, date) {
    ...
    this.readEvents = function() {
        ...
        for (var i = 0; i < eventItems.length; i++) {
            ...
            var id = (eventItems[i].getAttribute("id").split('timeline_event_id_'))[1];
    ...
    this.renderDay = function(topRow, bottomRow, data){
        ...
        if(data.events.length) {
            ...
            for (var i = 0; i < this.events.length; i++) {
                if(this.events[i].day == data.number) {
                    event_id_attr = " timeline_event_id=\"" + this.events[i].id + "\"";
                }
            };
        }

        
        topRow   .append("<li><div class=\"" + cssClass + "\" " + event_id_attr + ">" + data.name   + "</div></li>");
        bottomRow.append("<li><div class=\"" + cssClass + "\" " + event_id_attr + ">" + data.number + "</div></li>");
...
Timeline.Bubble = function(timeline) {
    ...
    this.setContent = function(events) {
        ...
        for (var i = 0; i < events.length; i++)
            html += "<div><div id=\"" + events[i].id + "\" class=\ event_title\">" + events[i].name + "<p class=\"event_data\">" + events[i].content + "</p></div></div>";
